...
// 나오지 않는다. 획 속은 위아래 픽셀이 둘 다 차서 반블록 위칸(▀)에
// 아래칸 색을 배경으로 깐 칸이 이어지므로, 그 조각으로 이름표가
// 그려졌는지만 본다.
//
// 꽉 찬 블록(█)이 아닌 이유는 본 글자에 세로 그라데이션이 깔려서다 —
// 한 칸의 위아래 픽셀이 서로 다른 색이면 글자색과 배경색을 함께 써야
// 하고, 그때 문자는 █ 이 아니라 ▀ 다(home.go renderWordmark).
