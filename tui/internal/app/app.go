...
type Fact struct {
	...
	// Off 는 "이건 켜면 되는데 아직 안 켰다"는 뜻이다.
	//
	// 첫 화면이 무엇을 권할지 정하는 데 쓴다 — 켤 것이 남아 있으면
	// "들어가라"가 아니라 "켜라"고 말해야 한다. 들어가서 써 보고 나서야
	// 안 되는 것을 아는 순서는 거꾸로다.
	//
	// 관문(Ready)과 다르다. 관문은 막힌 것이고 이건 안 켠 것이다 —
	// 안 켜도 앱은 그대로 돈다.
	Off bool

	// Fix 는 이것을 켜는 명령이다. Off 일 때만 채운다.
	//
	// 첫 화면이 **무엇을 치면 되는지**를 그대로 적을 수 있게 하려고 둔다.
	// "/ 를 눌러 보라"까지만 말하면, 눌렀을 때 팔레트 여덟 줄에 그 명령이
	// 없을 수도 있다 — 시킨 대로 했는데 시킨 것이 없는 화면이 된다.
	Fix string
}
...
type Command struct {
	...
	// Secret 은 이 인자가 비밀이라는 뜻이다. 호스트가 따로 물어보고,
	// 치는 동안 화면에 찍지 않는다.
	//
	// 명령 한 줄에 이어 치게 두면 API 키가 화면에 그대로 남는다. 시연이나
	// 화면 공유에서는 그것이 영구히 남는 기록이 된다. docker 가 --password
	// 를 쓰면 경고하는 것도 같은 이유다.
	//
	// 앱이 아니라 계약에 두는 이유는 **가리는 일이 입력창의 일**이기
	// 때문이다. 입력창은 호스트의 것이므로 앱은 이 값이 비밀이라고
	// 말만 하고, 어떻게 가릴지는 호스트가 정한다.
	Secret bool
}
