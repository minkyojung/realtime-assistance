...
// CycleMsg 는 "화면을 한 칸 넘겨"다.
//
// 앱 하나가 본문 자리를 여러 가지로 나눠 쓸 때(음악은 가사·대화·이력)
// 손으로 넘길 길이 있어야 한다. 저절로만 바뀌면 "그거 좀 다시 보자"를
// 할 방법이 없다.
//
// 호스트가 `ctrl+j` 를 여기 묶는다. 앱이 자기 키를 만들지 않는다는 계약
// (docs/07 3-1)을 지키면서 키를 주는 유일한 길이다 — 조작법은 호스트의
// 것이고, 그 키가 무슨 뜻인지는 앱이 정한다.
//
// 메서드가 아니라 메시지인 이유는 FocusMsg 와 같다. 나눠 쓸 자리가 없는
// 앱은 무시하면 된다.
