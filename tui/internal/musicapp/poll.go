...
type queueMsg struct {
	...
	// dropped 는 **담지 못한** 곡 수다. 로그인 전이거나 애플이 거절했다.
	dropped int

	// syncing 은 **담겼지만 아직 Music.app 에 안 나타난** 곡 수다.
	//
	// dropped 와 나누는 이유는 사용자가 할 일이 다르기 때문이다. 담지
	// 못한 것은 다시 시켜야 하고, 동기화 중인 것은 기다리면 온다.
	// 한때 둘을 함께 세어 "could not be added" 라고 말했는데, 곡은
	// 라이브러리에 멀쩡히 들어가 있었다. 거짓말은 없느니만 못하다.
	syncing int
