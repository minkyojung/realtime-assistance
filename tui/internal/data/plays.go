...
type Play struct {
	...
	// TurnID 는 어느 요청에서 나온 곡인가다(turns.go). 0 이면 사람이 목록에서
	// 직접 고른 것이다 — **그것도 사실이므로 빈 값이 아니라 뜻이 있는 값이다.**
	//
	// 맥락별 판단은 이것 없이 못 한다. "공부할 때는 락을 줄인다" 는 언제
	// 무엇을 청했는지를 알아야 나온다.
	TurnID int64 `json:"turnId,omitempty"`

	// Context 는 예전에 큐 제목을 베껴 적던 자리다. **더는 쓰지 않는다.**
	//
	// 지우지 않는 이유는 이미 적힌 줄들의 유일한 단서이기 때문이다. 새 줄에는
	// 안 들어가고(omitempty), 옛 줄을 읽을 때만 값이 있다.
