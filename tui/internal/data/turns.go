...
// **한 축이다 — 무엇을 하는 중인가.** 한때 여기에 `background`("틀어만 두는
// 것")가 있었는데, 그것은 활동이 아니라 **주의의 정도**다. 축이 둘 섞이면
// 반드시 겹친다 — 공부하며 틀어두는 것은 focus 이면서 background 다.
//
// 주의의 정도를 알고 싶으면 칸을 따로 내야 한다. 지금은 내지 않는다.
// 칸을 곱하면 조합마다 표본이 몇 개씩만 남고, 그러면 셀 수 있는 것이 없다.
var Contexts = []Context{
	"focus",   // 집중이 필요한 일 — 일 · 공부 · 읽기
	"workout", // 몸을 쓰는 중 — 운동 · 훈련
	"chores",  // 손은 바쁘고 머리는 한가한 일 — 집안일 · 요리 · 심부름
	"commute", // 어딘가로 가는 중
	"social",  // 곁에 사람이 있다
	"unwind",  // 깨어서 혼자 쉬는 중
	"sleep",   // 자려고
	"other",   // **요청이 자리를 말하지 않았다.** 음악을 보고 짐작하지 않는다
}
