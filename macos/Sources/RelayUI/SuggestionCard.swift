...
public struct SuggestionCard: View {
    ...
    public init(_ item: SuggestionItem) {
        self.item = item
    }
    ...
    private var mode: Mode? {
        switch item.mode {
        case .direct:      Mode(label: "바로 답변 가능", color: .green)
        case .conditional: Mode(label: "조건부 답변", color: .orange)
        case .escalate:    Mode(label: "확인 필요", color: .red)
        case nil:          nil
        }
    }
    ...
    private var header: some View {
        ...
        .accessibilityLabel(mode.map { "\($0.label) 제안" } ?? "판정을 기다리는 중")
    }

    private var headline: some View {
        // 판정 전에는 자리만 잡아 둔다. 뷰를 갈아끼우지 않고 `.redacted` 로 가리므로
        // 판정이 도착해 걷힐 때 레이아웃이 튀지 않는다.
        Text(item.headline.isEmpty ? "판정을 기다리는 중입니다" : item.headline)
    ...
    private var streamed: AttributedString {
        guard !item.script.isEmpty else { return AttributedString("제안 문구를 준비하고 있습니다") }
